...
[Step 2] [보고서 생성] 클릭
    ↓
[2단계 파이프라인] 경영진/실무/개발 피처/테스트 케이스 모두:
  1단계: 추출 md → LLM → 정리된 보고 내용(2차 가공 md)
  2단계: 정리 md + 선택 템플릿 → LLM → 완성 HTML
  (이미 정리 md가 있고 보고 유형이 같으면 HTML 형식만 바꿀 때 1단계 생략, 2단계만 재실행)
    ↓
[프론트엔드] 생성된 HTML 보기 / 정리된 마크다운(.md) 다운로드. 토큰·비용은 정리 md(1단계) + HTML(2단계) 합산 표시.
HTML 생성이 실패해도 정리 md가 있으면 해당 md는 유지·표시되며 다운로드 가능.
```
...
2. **[보고서 생성]** 버튼을 누르면, **경영진/실무/개발 피처/테스트 케이스** 모두 **2단계 파이프라인**으로 동작합니다.
   - **1단계** `generateRefinedMarkdownClient`: 추출 마크다운 → LLM(편집 가능 프롬프트 + 품질 루브릭) → **정리된 보고 내용(2차 가공 md)**.
   - **2단계** `generateHtmlFromMarkdownClient`: 정리 md + 선택 템플릿 → LLM(편집 가능 프롬프트 + HTML 고정 규칙 + 동적 포맷 지시) → **완성 HTML**.
3. **HTML 형식만 바꾼 뒤** 다시 [보고서 생성]하면, 이미 있는 정리 md와 보고 유형이 같을 때는 **정리 md를 다시 만들지 않고**, 기존 정리 md를 기준으로 선택한 HTML 포맷으로만 2단계를 다시 실행합니다.
4. 화면에 표시되는 **토큰 사용량·예상 비용**은 1단계(정리 md)와 2단계(HTML 생성) **합산**입니다.
5. **2단계(HTML 생성)가 실패**해도 1단계에서 생성된 정리 md가 있으면, 해당 정리 md는 유지·표시되며 「정리 내용 (.md) 다운로드」로 받을 수 있습니다. 이때 알림 문구로 "HTML 보고서 생성에 실패했습니다. 정리된 내용(.md)은 아래에서 다운로드할 수 있습니다."가 표시됩니다.
6. "만들어진 보고서 보기"로 팝업에서 HTML 보고서를 확인하고, 필요 시 HTML 또는 정리 내용(.md)을 다운로드합니다.
